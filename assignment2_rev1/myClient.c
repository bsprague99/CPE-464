...
void initial_packet(int socket, char *handle)
{
	...
	if ((length = send(socket, sendBuff, header.len, 0)) < 0) {
        perror("error in send");
        // return -1;
        exit(-1);
    }
	if ((length = recv(socket, &header, sizeof(header), MSG_WAITALL)) < 0)
	{
	...
	if ((length = recv(socket, &handle, header.len - 3, MSG_WAITALL)) < 0)
	{
		perror("Error in payload");
		// return -1;
		exit(-1);
